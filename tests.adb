--  Standalone test suite for Kruskals_Algorithm (main program).
...
procedure Tests is
   ...
   G, G2          : Graph;
   Tree, Tree_P   : Edge_List (1 .. Max_Edges);
   C, CP          : Natural;
   W, WP          : Weight_Sum;
   Has            : Boolean;
   ...
   procedure Agree_With_Prim (Label : String) is
   begin
      Minimum_Spanning_Tree (G, Tree, C, W);
      Prim_Reference (G, Tree_P, CP, WP);
      Check (C = CP, Label & " count agree");
      Check (W = WP, Label & " weight agree");
   end Agree_With_Prim;
...
begin
   ...
   Kruskal (G, Tree, C, W);
   Check (C = 0 and then W = 0, "empty Kruskal empty");
   Prim_Reference (G, Tree_P, CP, WP);
   Check (CP = 0 and then WP = 0, "empty Prim empty");
   ...
   Agree_With_Prim ("textbook");
   Kruskal (G, Tree, C, W);
   Check (C = 3 and then W = 6, "alias Kruskal same");
   ...
   Agree_With_Prim ("two-vert");

   Clear (G, 2);
   Add_Edge (G, 1, 2, 0);
   Minimum_Spanning_Tree (G, Tree, C, W);
   Check (C = 1 and then W = 0, "zero-weight edge kept");
   Agree_With_Prim ("zero-wt");
   ...
   Agree_With_Prim ("self-loops");

   Clear (G, 1);
   Add_Edge (G, 1, 1, 5);
   Minimum_Spanning_Tree (G, Tree, C, W);
   Check (C = 0 and then W = 0, "only self-loop → empty tree");
   Agree_With_Prim ("only-loop");
   ...
   Agree_With_Prim ("forest");

   Clear (G, 3);
   --  three isolated
   Minimum_Spanning_Tree (G, Tree, C, W);
   Check (C = 0 and then W = 0, "three isolated");
   Agree_With_Prim ("isolated3");
   ...
   Add_Edge (G, 1, 3, 5);
   Minimum_Spanning_Tree (G, Tree, C, W);
   Kruskal (G, Tree_P, CP, WP);
   Check (C = CP and then W = WP, "alias totals");
   ...
   for N in 2 .. 8 loop
      ...
      Agree_With_Prim ("path-N" & Integer'Image (N));
   end loop;
   ...
   for N in 3 .. 7 loop
      ...
      Agree_With_Prim ("cyc-N" & Integer'Image (N));
   end loop;
   ...
   Minimum_Spanning_Tree (G, Tree, C, W);
   Prim_Reference (G, Tree_P, CP, WP);
   Check (C = 3 and then CP = 3, "dup count 3");
   Check (W = 3 and then WP = 3, "dup weight 3");
   Check (W = WP, "dup totals match");

   ------------------------------------------------------------------
   Section ("18. Light-first addition order stress");
   ------------------------------------------------------------------
   Clear (G, 5);
   Add_Edge (G, 1, 2, 100);
   Add_Edge (G, 2, 3, 90);
   ...
   Agree_With_Prim ("zero-ties");

   ------------------------------------------------------------------
   Section ("23. Prim alone smoke");
   ------------------------------------------------------------------
   Clear (G, 4);
   Add_Edge (G, 1, 2, 4);
   Add_Edge (G, 2, 3, 1);
   ...
   Add_Edge (G, 1, 4, 2);
   Prim_Reference (G, Tree_P, CP, WP);
   Check (CP = 3, "Prim alone count");
   Check (WP = 6, "Prim alone weight 1+2+3");
   Minimum_Spanning_Tree (G, Tree, C, W);
   Check (W = WP and then C = CP, "Kruskal matches Prim alone");
   ...
   Agree_With_Prim ("bridged-tri");

   Kruskal (G, Tree, C, W);
   Prim_Reference (G, Tree_P, CP, WP);
   Check (C = CP and then W = WP, "final alias=Prim");
